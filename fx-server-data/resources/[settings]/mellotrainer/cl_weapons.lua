...
function toggleWeaponEquipped(weaponName)
	...
	local playerName = GetPlayerName(PlayerId())
	    ...
	    TriggerServerEvent('DiscordBot:ToDiscord', 'chat', 'WSRP Log | Weapons', '```css\n'..playerName..' взял '..weaponName..' ('..ammoAmmount ..'/'..addClip..')[1]\n```', 'https://wiki.gtanet.work/images/8/8b/weapon_-2084633992.png', true)
            return
        elseif(ammoAmmount < addClip) then
            SetPedAmmoByType(playerPed, ammoType, addClip)
        end
	TriggerServerEvent('DiscordBot:ToDiscord', 'chat', 'WSRP Log | Weapons', '```css\n'..playerName..' взял '..weaponName..' ('..ammoAmmount ..'/'..addClip..')[2]\n```', 'https://wiki.gtanet.work/images/8/8b/weapon_-2084633992.png', true)
