...
admins = {
    'steam:11000010365bcb4',
    'steam:11000010892e5fa',
    'steam:11000010f38512f',
	'steam:11000010f2ee4d0',
}
...
RegisterCommand('freezetime', function(source, args)
    if source ~= 0 then
        ...
        freezeTime = not freezeTime
        if freezeTime then
            print("Время теперь заморожено.")
        else
            print("Время разморожено.")
        end
    end
end)
...
RegisterCommand('freezeweather', function(source, args)
    if source ~= 0 then
        ...
        DynamicWeather = not DynamicWeather
        if not DynamicWeather then
            print("Погода заморожена.")
        else
            print("Погода разморожена.")
        end
    end
end)
...
RegisterCommand('time', function(source, args, rawCommand)
    if source == 0 then
        if tonumber(args[1]) ~= nil and tonumber(args[2]) ~= nil then
            ...
            print("Время изменено " .. argh .. ":" .. argm .. ".")
            TriggerEvent('vSync:requestSync')
        else
